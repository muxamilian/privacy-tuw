...
Evidently, the larger $c$, the better the attack works, but also distances from original flows become higher. With acceptable distances, we were able to generate adversarial samples for about 80\% of attack samples.

Note, however, that for successful adversarial samples, the second term in \autoref{eq:carliniWagner} becomes constant and, hence, is no longer relevant for optimization. Hence, $c$ is irrelevant for the achieved distance, but governs if for one sample an adversarial sample is found.
However, to avoid overly large steps when using a large $c$ in \autoref{eq:carliniWagner}, it is necessary to reduce the gradient descent learning rate. Since the distance term is not affected by $c$, it is furthermore necessary to scale the iteration count inversely with the learning rate. \gls{cw} with a large $c$ therefore needs more time than with a small $c$.
...
Such a metric should be easy to compute, easy to interpret and robust with respect to outliers. We thus propose the \gls{ars} as the average distance of the adversarial samples to the original samples for the 50\% of the samples which an adversary could successfully modified and which have the smallest distances. Since \gls{cw} is able to find adversarial samples with minimum distance, it can be used for finding the \gls{ars} efficiently. 

Since \gls{cw} becomes slow for large $c$, the \gls{ars} can be efficiently found as follows: Use \gls{cw} with a small tradeoff, trying to generate adversarial samples for an attack type. If, of a total of $N$ samples,
...
stop. Otherwise increase $c$ and repeat. If after a predefined number of iterations -- e.g. 100 -- still not more than half of the samples are adversarial, also break.


The result is a number of adversarial samples and some that could not be made adversarial. If at least half of the samples are misclassified, we sort the adversarial samples by ascending distance to the original samples. Then we take samples by ascending distance until we took 50\% of all samples. Of these samples, the distance of each adversarial sample to the original sample is taken and averaged. The resulting average distance is the \gls{ars}. The larger it is the better since then an adversary needs to modify the samples more to make them adversarial. If not more than half of the samples could be made adversarial, our metric is $\infty$ since then apparently it is not possible for an adversary to reliably conduct adversarial attacks on the majority of samples. Then the ratio of samples that could be made adversarial ($<$50\%) can be a useful metric to determine the exact extent of the adversarial threat. Setting the threshold for the \gls{ars} to 50\% is arbitrary, but
...
this does not hold true and a feature might influence a decision many time steps later.
\end{itemize}
