...
However, to avoid numerical issues, when using a large $c$ in \autoref{eq:carliniWagner}, it is necessary to reduce the gradient descent learning rate. Since the distance term is not affected by $c$, it is furthermore necessary to scale the iteration count inversely with the learning rate. \gls{cw} with a large $c$ therefore needs more time than with a small $c$.
