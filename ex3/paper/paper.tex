...
\label{sec:adv}
We now investigate whether known \gls{aml} methods can be used for generating adversarial flows for our \gls{rnn}..
...
We used $L_1$ norm as distance metric and \gls{pgd} for meeting the real-world constraints discussed above.
...
% \toprule
% Attack type & \multicolumn{2}{l}{FGSM} & \multicolumn{2}{l}{Carlini-Wagner, $L_1$} & \multicolumn{2}{l}{Carlini-Wagner, $L_2$} & \multicolumn{2}{l}{$L_\infty$-bounded \gls{pgd}} \\
...
%\caption{Performance of FSGM and $L_\infty$-bounded \gls{pgd} for different bounds.
%\todo{Max: Omit}}
%\label{fig:fgsm}
%\end{figure*}
...
%
%To this end, we generated adversarial flows with the \gls{cw} with $L_2$ distance and a tradeoff of 0.5. \autoref{adv_plots} depicts how the method manipulates flows for some exemplary attack categories. Depicted is the mean of both unmodified flows and adversarial flows  of samples of individual attack families considering both \gls{iat} and packet length.
%
...
Such a metric should be easy to compute, easy to interpret and robust with respect to outliers.  We thus propose the \gls{ars}, as the average of the lower half  of minimum distances of adversarial samples. Since \gls{cw} is able to find adversarial samples with minimum distance, the \gls{ars} can 
  be found efficiently as follows: Use \gls{cw} with a small tradeoff to try to generate adversarial samples for an attack type. If more than half of the samples are now wrongly classified, stop. Otherwise increase the tradeoff of \gls{cw} and repeat. If after a predefined number of iterations -- e.g. 100 -- still not more than half of the samples are adversarial, also break. 

\todo{Alex: I think this is quite technical curretnly and can be shortened a bit}
...
\caption{Recall for each attack type and average modification per flow required by an adversary to make a flow appear benign for CIC-IDS-2017.}
...
Denoting as $X=\{\boldsymbol X^1, \ldots, \boldsymbol X^m\}$ the series of packet features $\boldsymbol X^t \in \mathbb R^n$ and $h_t(X)$ the network's hidden state at time step $t$, we define the sequential \gls{pdp} as 
\begin{align}
\text{PD}&\text{P}_{c,i}(t,w)= \\ &\mathbb E_{X | C} \Big(f \left(h_{t-1}( X), X_1^t,\ldots,X_{i-1}^t,w,X_{i+1}^t,\ldots X_n^t \right) | c\Big). \nonumber 
\end{align}
...
After 50 epochs of training, accuracy is essentially identical to results presented in \autoref{tab:performance_results}. Recall and Youden increased and precision slightly decreased. However, this is due to the higher proportion of attack samples in training data.
