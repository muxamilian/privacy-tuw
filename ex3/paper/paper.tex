...
With the recent rise of interest in \gls{aml} techniques, also \gls{aml} for \glspl{ids} have been investigated. E.g. \cite{Bachl2019} investigates remedies for poisoning attacks on \glspl{ids}.
...
Surprisingly, we can confirm that adversarial samples can be found even when considering thse tight constraints. % mentioned before.
...
Astonishingly, feature importance methods reveal that the manipulable features, that are left for adversaries to modify are not even particularly significant for the classification outcome. %Especially the first packets of a flow are critical for determining the decision of a recurrent \gls{ids}. 
