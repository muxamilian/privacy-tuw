...
$X=\{\boldsymbol X^1, \ldots, \boldsymbol X^m\}$

\begin{align}
\begin{split} \label{eq:pdp_conditional}
&\text{PDP}_{c,i}(t,w)= \\ &\mathbb E_{X | C} \Big(f \left(h_{t-1}( X), X_1^t,\ldots,X_{i-1}^t,w,X_{i+1}^t,\ldots X_n^t \right) | c\Big),
\end{split}
\end{align}
...
\caption{Exemplary PD plot and adversarial flows for the DoS Slowloris attack in CIC-IDS-2017. The solid line shows the mean value of the feature for all flows of the attack type. The shaded region shows the change in confidence that occurs when the feature is varied.}
