...
\todo{update numbers, shorten attack names, What's the tradeoff for CW?}
\end{figure}
...
As \autoref{fig:adv_per_family} shows, for most attack types, most samples can be modified to look benign by an adversary by using \gls{cw}. High accuracy for a particular attack category does not imply that adversarial attacks are more likely to fail for these attacks. Thus we argue that beside the classical metrics such as accuracy, false positives etc., a new metric is needed, which quantifies how easily an adversary can fool the classifier. 
...
\includegraphics[width=0.48\textwidth]{{"../plots/plot/flows_pred_plots2_outcomes_0_3.pickle_6_DoS - DDoS-DoS Slowhttptest"}.pdf}
...
\caption{The solid line shows the mean value of the feature for all flows of the attack type. The shaded region shows the change in confidence that occurs when the feature is varied.}
...
\todo{Max: Maybe here two figures are enough?}
